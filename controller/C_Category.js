...
exports.Resize = AsyncHandler(async (req, res, next) => {
  const filename =`Category_${uuidv4()}.jpeg`
  if (req.file) {
...
exports.Create = AsyncHandler(async (req, res) => {
  ...
  const data = await M_Category.create(req.body)
...
exports.Get = AsyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    ...
    paginationResult: {
      currentPage: page,
      numberOfPages: Math.ceil(await M_Category.find().countDocuments() / limit),
      limit
    },
    data
  })
...
exports.Get_Id = AsyncHandler(async (req, res , next) => {
  const data = await M_Category.findById(req.params.id)
...
exports.Update = AsyncHandler(async (req, res , next) => {
  ...
  const data = await M_Category.findByIdAndUpdate(req.params.id, req.body , { new: true })
  if(!data) next( new Error("This user is not found"))
  res.status(200).json({data})
})

exports.Delete = AsyncHandler(async (req, res , next) => {
  const data = await M_Category.findByIdAndDelete(req.params.id)
  if(!data) next( new Error("This user is not found"))
